...
module Module.Zap.Untyped.Pure (pureUntypedTests) where

import Zap (msgToRaw, parse)
import Zap.Basics
import Zap.TraversalLimit (runLimitT)
import GHC.Float (castDoubleToWord64)
import Test.Hspec
import Text.Heredoc (here)
import Util

-- This is analogous to Tests.Module.Zap.Untyped.untypedTests, but
-- using the Pure module:
pureUntypedTests :: Spec
pureUntypedTests =
  describe "high-level untyped decoding" $
    it "Should agree with `zap decode`" $ do
      msg <-
        encodeValue
          aircraftSchemaSrc
          "Aircraft"
