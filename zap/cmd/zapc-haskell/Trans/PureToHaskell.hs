...
commonImports =
  [ ImportAs {importAs = "V", parts = reExp ["Data", "Vector"]},
    ...
    ImportAs {importAs = "UntypedPure", parts = ["Zap", "Untyped", "Pure"]},
    ImportAs {importAs = "Untyped", parts = ["Zap", "Untyped"]},
    ImportAs {importAs = "Message", parts = ["Zap", "Message"]},
    ImportAs {importAs = "Classes", parts = ["Zap", "Classes"]},
    ImportAs {importAs = "BasicsPure", parts = ["Zap", "Basics", "Pure"]},
    ImportAs {importAs = "GenHelpersPure", parts = ["Zap", "GenHelpers", "Pure"]}
  ]

-- | Modules imported by generated modules that use rpc.
rpcImports :: [Import]
rpcImports =
  [ ImportAs {importAs = "Rpc", parts = ["Zap", "Rpc", "Untyped"]},
    ImportAs {importAs = "Server", parts = ["Zap", "Rpc", "Server"]},
    ImportAs {importAs = "RpcHelpers", parts = ["Zap", "GenHelpers", "Rpc"]},
...
reExp parts = ["Zap", "GenHelpers", "ReExports"] ++ parts
...
fileToModuleAlias P.File {fileName, fileId} =
  let reExport = ["Zap", "Gen"] ++ makeModName fileName ++ ["Pure"]
   in Module
...
fileToMainModule P.File {fileName, fileId, decls, reExportEnums, usesRpc} =
  fixImports $
    Module
      { modName = ["Zap", "Gen"] ++ makeModName fileName ++ ["Pure"],
        modLangPragmas =
          [ "DataKinds",
            "DeriveGeneric",
...
declToExport fileId = \case
  ...
  P.ConstDecl P.Constant {name, value = C.PtrValue _ _} ->
    [ExportLName name]
  P.IFaceDecl P.IFace {name = Name.ZapQ {local}} ->
    [ ExportLCtors local,
...
ifaceClientDecl _thisMod P.IFace {name = Name.ZapQ {local = name}, typeParams} =
  DcData
    Data
...
ifaceClassDecl thisMod P.IFace {name = Name.ZapQ {local = name}, methods, supers, typeParams} =
        ...
        { ctx =
            let superConstraints =
                  ...
                  [ let superClass = pureTName thisMod fileId (Name.mkSub local "server_")
                    ...
                    | (P.IFace {name = Name.ZapQ {local, fileId}}, C.ListBrand types) <- supers
                  ]
...
ifaceExportFn thisMod iface@P.IFace {name = Name.ZapQ {local}, typeParams, ancestors} =
                                    ...
                                    ELambda [PVar "interfaceId_", PVar "methodId_"] $
                                      ECase (euName "interfaceId_") $
                                        [ ( PInt (fromIntegral interfaceId),
                                          ...
                                          | P.IFace
                                              { name = Name.ZapQ {local = ifaceName, fileId = ifaceMod},
                                                interfaceId,
                                                methods
                                              } <-
...
ifaceInstances thisMod iface@P.IFace {name = Name.ZapQ {local = name}, typeParams} =
...
ifaceServerInstances thisMod iface@P.IFace {name = Name.ZapQ {local = name}, typeParams, ancestors} =
  DcInstance
    { ctx = [],
  ...
  where
    ...
    go (P.IFace {name = Name.ZapQ {local, fileId}, interfaceId, methods}, C.ListBrand types) =
...
data MarshalField = MarshalField
  { -- | The id for the module we're generating
    ...
    from :: Name.UnQ,
    type_ :: C.Type (C.ListBrand Name.ZapQ) Name.ZapQ,
...
typeToType :: Word64 -> C.Type (C.ListBrand Name.ZapQ) Name.ZapQ -> Type
typeToType _thisMod C.VoidType =
  TUnit
typeToType _thisMod (C.WordType (C.PrimWord ty)) =
  TPrim ty
typeToType thisMod (C.WordType (C.EnumType Name.ZapQ {local, fileId})) =
...
nameToType :: Word64 -> Name.ZapQ -> C.ListBrand Name.ZapQ -> Type
nameToType thisMod Name.ZapQ {local, fileId} (C.ListBrand args) =
  let head
...
rawModule modId =
  ["Zap", "Gen", "ById", T.pack $ printf "X%x" modId]
