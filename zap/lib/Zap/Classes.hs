...
-- useful for operating over Zap values.
module Zap.Classes
...
-- | Zap types that can be parsed into a more "natural" Haskell form.
--
-- * @t@ is the zap type.
...
-- | Types which have a numeric type-id defined in a zap schema.
...
-- | If @a@ is a zap type, then @Parsed a@ is an ADT representation of that
