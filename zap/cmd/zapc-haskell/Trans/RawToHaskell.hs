  ...
  fixImports $
    Module
      { modName = ["Zap", "Gen"] ++ makeModName fileName,
        ...
        modExports = Nothing,
        modImports =
          [ imp ["Zap", "Message"] "Message",
            imp ["Zap", "Untyped"] "Untyped",
            imp ["Zap", "Basics"] "Basics",
            imp ["Zap", "GenHelpers"] "GenHelpers",
            imp ["Zap", "Classes"] "Classes",
            imp ["GHC", "Generics"] "Generics",
            -- So we can treat 'Word1' the same as 'Word16' etc.
            imp ["Zap", "Bits"] "Std_",
            imp ["Data", "Maybe"] "Std_",
            imp ["Zap", "GenHelpers", "ReExports", "Data", "ByteString"] "BS"
          ],
...
eSetValue :: C.FieldLocType (C.ListBrand Name.ZapQ) Name.ZapQ -> Exp
eSetValue = \case
...
nameToType :: Word64 -> Name.ZapQ -> Type
nameToType thisMod Name.ZapQ {local, fileId} =
  if fileId == thisMod
    then TLName local
    else
      tgName
...
typeToType :: Word64 -> C.Type (C.ListBrand Name.ZapQ) Name.ZapQ -> Type -> Type
typeToType thisMod ty msgTy = case ty of
  ...
  where
    appV t = TApp t [msgTy]
    namedType :: Name.ZapQ -> C.ListBrand Name.ZapQ -> Type
