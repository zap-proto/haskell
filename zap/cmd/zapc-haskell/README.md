...
do `zap compile -o haskell`. This document provides a high-level
overview of its internals.

Like all zap code generator plugins, it accepts a
zap-serialized `CodeGenratorRequest` (defined in `schema.zap`)
on standard input, and emits code accordingly. For an overview on *what*
exactly is emitted, see `Zap.Tutorial`.
...
* Next, we translate to the `AbstractOp` IR. This form encodes the
  ...
  * Type declaration for this zap type
