...
-- Description: Translate from schema.zap's codegenerator request to IR.Stage1.
module Trans.CgrToStage1 (cgrToCgr) where

import qualified Zap.Basics as B
import Zap.Classes (toWord)
import Zap.Fields (HasUnion (..))
...
-- | Given the offset field from the zap schema, a type, and a
