...
-- |
-- Module: Zap.Basics
-- Description: Handling of "basic" zap datatypes.
--
-- This module contains phantom types for built-in Zap
-- types, analogous to the phantom types generated for structs
-- by the code generator. It also defines applicable type class
-- instances.
module Zap.Basics where
...
-- | A Zap @AnyPointer@, i.e. an arbitrary pointer with unknown schema.
data AnyPointer

-- | A Zap @List@ with unknown element type.
data AnyList

-- | A Zap struct of unknown type.
data AnyStruct

-- | A Zap capability with unknown interfaces.
data Capability
