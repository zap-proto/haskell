...
module Module.Zap.Rpc (rpcTests) where
...
import Zap.Bits (WordCount)
import Zap.Gen.Aircraft hiding (Left, Right)
import Zap.Gen.Zap.Rpc
import qualified Zap.Gen.Echo as E
import Zap.Mutability (freeze)
import qualified Zap.Pointer as P
import qualified Zap.Repr as R
import Zap.Rpc hiding (Client)
import Zap.Rpc.Errors (eFailed)
import Zap.Rpc.Revoke (makeRevocable)
import Zap.Rpc.Untyped hiding (Client, Pipeline, export, waitPipeline)
...
aircraftTests :: Spec
aircraftTests = describe "aircraft.zap rpc tests" $ do
...
-- zap, and others just won't come up when talking to the Haskell
