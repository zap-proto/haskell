...
imports :: [Hs.Import]
imports =
  [ Hs.ImportAs {importAs = "R", parts = ["Zap", "Repr"]},
    Hs.ImportAs {importAs = "RP", parts = ["Zap", "Repr", "Parsed"]},
    Hs.ImportAs {importAs = "Basics", parts = ["Zap", "Basics"]},
    Hs.ImportAs {importAs = "OL", parts = ["GHC", "OverloadedLabels"]},
    Hs.ImportAs {importAs = "GH", parts = ["Zap", "GenHelpers"]},
    Hs.ImportAs {importAs = "C", parts = ["Zap", "Classes"]},
...
-- avoid a circular import when generating code for rpc.zap -- which does not
...
rpcImports =
  [ Hs.ImportAs {importAs = "GH", parts = ["Zap", "GenHelpers", "Rpc"]}
  ]
...
fileToMainModule file@AO.File {fileName, usesRpc} =
  fixImports $
    Hs.Module
      { modName = ["Zap", "Gen"] ++ makeModName fileName,
        modLangPragmas =
          [ "TypeFamilies",
            "DataKinds",
            "DeriveGeneric",
...
fileToModuleAlias AO.File {fileId, fileName} =
  let reExport = ["Zap", "Gen"] ++ makeModName fileName
   in Hs.Module
...
-- | Constraints required for a zap type parameter. The returned
-- expression has kind 'Constraint'.
paramConstraints :: Hs.Type -> Hs.Type
paramConstraints t =
  Hs.TApp (tgName ["GH"] "TypeParam") [t]

tZap :: Word64 -> Name.ZapQ -> Hs.Type
tZap thisMod Name.ZapQ {local, fileId}
  | thisMod == fileId = Hs.TLName local
  | otherwise = tgName (map Name.renderUnQ $ idToModule fileId) local

fieldLocTypeToType :: Word64 -> C.FieldLocType AO.Brand Name.ZapQ -> Hs.Type
...
namedType :: Word64 -> Name.ZapQ -> C.ListBrand Name.ZapQ -> Hs.Type
namedType thisMod name (C.ListBrand []) = tZap thisMod name
...
emitMarshalField :: Name.UnQ -> C.FieldLocType AO.Brand Name.ZapQ -> Hs.Exp
...
emitMarshalVariant :: Name.UnQ -> C.FieldLocType AO.Brand Name.ZapQ -> Hs.Exp
