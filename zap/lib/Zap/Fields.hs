...
-- in the @Field@ type in schema.zap. Mostly used at the type level with
...
-- | An instance of 'HasUnion' indicates that the given type is a zap struct
...
class R.IsStruct a => HasUnion a where
  ...
  -- | 'Which' is the abstract zap type of the union itself. Like
