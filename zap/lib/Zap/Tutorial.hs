...
-- |
-- Module: Zap.Tutorial
-- Description: Tutorial for the Haskell Zap library.
--
-- This module provides a tutorial on the overall usage of the library. Note that
-- it does not aim to provide a thorough introduction to zap itself; see
-- <https://zap-proto.io> for general information.
--
...
-- $overview
--
-- This module provides an overview of the zap library.

-- $setup
--
-- In order to generate code from schema files, you will first need to make
-- sure the @zap@ and @zapc-haskell@ binaries are in your @$PATH@. The
-- former ships with the zap reference implementation; see
-- <https://zap-proto.io/docs/install>. The latter is included with this
-- library; to install it you can run the command:
--
-- > cabal install zap --installdir=$DIR
--
-- which will compile the package and create the @zapc-haskell@ executable
-- at @$DIR/zapc-haskell@.
...
-- $highlevel
--
-- The high level API exposes zap values as regular algebraic data
-- types.
--
-- On the plus side:
--
-- * This makes it easier to work with zap values using idiomatic
--   Haskell code
...
-- namespaces within a single zap schema.
--
-- So, we see that zapc-haskell generates:
--
...
--     ADT corresponding to the structure of the zap type.
...
--
-- The module "Zap" exposes the most frequently used
-- functionality from the zap package. We can write an address book
...
-- > import Zap (getParsed, defaultLimit)
-- >
-- > -- ...
-- >
-- > main = do
...
-- "Zap.TraversalLimit" for more information. 'getValue' uses this
...
-- and a phantom type indicating the zap type. This second type parameter
-- will be instantiated with the (for structs, uninhabited) type generated by
-- the schema compiler plugin. The accessors in "Zap.Accessors"
-- (re-exported by "Zap") are used to read and write the fields.
...
-- `MonadLimit`, which is defined in "Zap.TraversalLimit". We define
...
-- $rpc
--
-- This package supports level 1 Zap RPC. The tuotrial will demonstrate the most
...
--
-- Note that zap does not have a notion of "clients" and "servers" in the
-- traditional networking sense; the two sides of a connection are symmetric. In
-- zap terminology, a "client" is a handle for calling methods, and a "server"
...
-- Zap interface's rpc methods. The handler has this type:
--
...
--
-- Once you have an instance of the server class, the 'Zap.export' function
...
-- >           }
--
-- For RPC clients, there is a 'Client' type exported by "Zap", which is
...
--
-- Actually invoking methods uses the functions in "Zap.Repr.Methods",
-- re-exported by "Zap". 'callP', 'callB', and 'callR' provide different
