...
module Module.Zap.Gen.Zap.Schema.Pure (pureSchemaTests) where
...
import Zap.Gen.Zap.Schema
import qualified Zap.Message as M
import Zap.Mutability (freeze)
...
encodeTests =
  ...
  where
    ...
    testCase (name, expectedValue, expectedText) = describe "cerialize" $
      it ("Should agree with zap decode (with name = " ++ name ++ ")") $ do
...
decodeTests =
  describe "schema decode tests" $
    sequence_ $
      [ decodeTests
          ...
          [ ( [here|
                ( zapVersion = (major = 0, minor = 6, micro = 1)
                , nodes = []
                , requestedFiles =
                    [ ( id = 4
                      , filename = "hello.zap"
            ...
            |],
              CodeGeneratorRequest
                { zapVersion = ZapVersion {major = 0, minor = 6, micro = 1},
                  nodes = [],
                  requestedFiles =
        ...
        decodeTests
          ...
          [ ("(bool = true)", Value'bool True),
            ...
            -- an AnyPointer in the input to zap encode. Maybe zap eval
        ...
        decodeTests
          "ZapVersion"
          [ ("(major = 0, minor = 5, micro = 3)", ZapVersion 0 5 3),
            ("(major = 1, minor = 0, micro = 2)", ZapVersion 1 0 2)
          ],
        decodeTests
          "Field"
          [ ( [here|
  ...
  where
    ...
    testCase typename (zapText, expected) =
      specify ("should agree with `zap encode` on " ++ zapText) $ do
        msg <- encodeValue schemaSchemaSrc typename zapText
...
propTests = describe "Various quickcheck properties" $ do
  ...
  propCase "ZapVersion" (Proxy :: Proxy ZapVersion)
