...
import qualified Zap as Zap
import qualified Zap.Basics as B
import Zap.Classes (encode)
...
idToModule fileId =
  ["Zap", "Gen", "ById", Name.UnQ $ T.pack $ printf "X%x" fileId]
...
-- | Fix the zap imports of a module, so that they correspond to the
-- imports actually used in the body of the module and/or export list.
--
-- Note that this only looks at imports of the form Zap.Gen....; other
  ...
  let namespaces =
        S.toList $
          S.fromList -- deduplicate
            [ nsParts
              | Name.GlobalQ
                  { globalNS = Name.NS nsParts@(map T.unpack -> "Zap" : "Gen" : _)
                  } <-
...
makePtrBytes ptr =
  Zap.msgToLBS $ fromJust $ Zap.createPure Zap.defaultLimit $ do
    msg <- Zap.newMessage Nothing
    rootPtr <-
      encode msg $
        B.Struct
          (fromList [])
