...
-- |
-- Module: Zap.Message
-- Description: Zap messages
--
-- This module provides support for working directly with Zap messages.
module Zap.Message
  ( Message,
    Segment,
    Mutability (..),
...
import Zap.Address (WordAddr (..))
import Zap.Bits (WordCount (..), hi, lo)
import qualified Zap.Errors as E
import Zap.Mutability (MaybeMutable (..), Mutability (..))
import Zap.TraversalLimit (LimitT, MonadLimit (invoice), evalLimitT)
...
-- | A Zap message, parametrized over its mutability.
...
-- | A 'Message' is a (possibly read-only) zap message. It is
...
-- | A 'MutMsg' is a mutable zap message. The type parameter @s@ is the
