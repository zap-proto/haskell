-- |
-- Module: Zap.Pointer
-- Description: Support for parsing/serializing zap pointers
--
-- This module provides support for parsing and serializing zap pointers.
...
-- | A 'Ptr' represents the information in a zap pointer.
data Ptr
...
-- | @'parsePtr' word@ parses word as a zap pointer. A null pointer is
-- parsed as 'Nothing'.
parsePtr :: Word64 -> Maybe Ptr
parsePtr 0 = Nothing
parsePtr p = Just (parsePtr' p)

-- | @'parsePtr'' word@ parses @word@ as a zap pointer. It ignores
