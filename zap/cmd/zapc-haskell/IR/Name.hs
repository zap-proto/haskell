...
instance HasUnQ ZapQ where
  getUnQ ZapQ {local} = getUnQ local
...
-- | A fully qualified name for something defined in a zap schema.
-- this includes a local name within a file, and the file's zap id.
data ZapQ = ZapQ
  { local :: LocalQ,
    fileId :: !Word64
  }
...
instance MkSub ZapQ where
  mkSub ZapQ {local, ..} unQ = ZapQ {local = mkSub local unQ, ..}
