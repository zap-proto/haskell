...
module Zap.Gen.Zap.Schema where
import qualified Zap.Repr as R
import qualified Zap.Repr.Parsed as RP
import qualified Zap.Basics as Basics
import qualified GHC.OverloadedLabels as OL
import qualified Zap.GenHelpers as GH
...
instance (C.TypedStruct ZapVersion) where
    numStructWords  = 1
    numStructPtrs  = 0
instance (C.Allocate ZapVersion) where
    type AllocHint ZapVersion = ()
    new _ = C.newTypedStruct
instance (C.EstimateAlloc ZapVersion (C.Parsed ZapVersion))
instance (C.AllocateList ZapVersion) where
    type ListAllocHint ZapVersion = Std_.Int
    newList  = C.newTypedStructList
instance (C.EstimateListAlloc ZapVersion (C.Parsed ZapVersion))
data instance C.Parsed ZapVersion
    = ZapVersion 
        {major :: (RP.Parsed Std_.Word16)
        ,minor :: (RP.Parsed Std_.Word8)
        ,micro :: (RP.Parsed Std_.Word8)}
    deriving(Generics.Generic)
deriving instance (Std_.Show (C.Parsed ZapVersion))
...
instance (GH.HasField "major" GH.Slot ZapVersion Std_.Word16) where
    fieldByLabel  = (GH.dataField 0 0 16 0)
instance (GH.HasField "minor" GH.Slot ZapVersion Std_.Word8) where
    fieldByLabel  = (GH.dataField 16 0 8 0)
instance (GH.HasField "micro" GH.Slot ZapVersion Std_.Word8) where
...
data instance C.Parsed CodeGeneratorRequest
    = CodeGeneratorRequest 
        ...
        ,zapVersion :: (RP.Parsed ZapVersion)
...
instance (C.Marshal CodeGeneratorRequest (C.Parsed CodeGeneratorRequest)) where
    marshalInto raw_ CodeGeneratorRequest{..} = (do
        ...
        (GH.encodeField #zapVersion zapVersion raw_)
...
instance (GH.HasField "requestedFiles" GH.Slot CodeGeneratorRequest (R.List CodeGeneratorRequest'RequestedFile)) where
    fieldByLabel  = (GH.ptrField 1)
instance (GH.HasField "zapVersion" GH.Slot CodeGeneratorRequest ZapVersion) where
    fieldByLabel  = (GH.ptrField 2)
