...
import Zap.Accessors
import qualified Zap.Basics as B
import Zap.Bits (WordCount, bytesToWordsFloor)
import Zap.Classes (new, newRoot, parse)
import Zap.Convert (msgToRaw, parsedToMsg)
import Zap.Fields (Which)
import qualified Zap.Gen.Zap.Rpc as R
import Zap.Message (Message)
import qualified Zap.Message as Message
import Zap.Mutability (Mutability (..), thaw)
import Zap.Repr (Raw (..), ReprFor)
import Zap.Rpc.Errors
  ( eDisconnected,
    eFailed,
...
import Zap.Rpc.Transport (Transport (recvMsg, sendMsg))
import Zap.TraversalLimit (LimitT, defaultLimit, evalLimitT)
import qualified Zap.Untyped as UntypedRaw
...
-- * The "Tribble 4-way Race Condition" as documented in rpc.zap. This
...
-- to design a module structure that avoids low-level zap serialization code
...
-- and a couple other things, and "Zap.Message" and a few other
...
handleCallMsg conn callMsg = do
  ...
  let zapMsg = UntypedRaw.message @(Raw R.Call) callMsg

  -- Apply backpressure, by limiting the memory usage of outstanding call
  -- messages.
  msgWords <- Message.totalNumWords zapMsg
  lift $ do
...
--   we mitigate these concerns when dealing with plain zap data (i.e. not
--   rpc).
