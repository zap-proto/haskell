...
-- representations of zap objects at the type level. The most
-- central part of this module is the 'Repr' type.
--
-- Recommended reading: https://zap-proto.io/docs/encoding
module Zap.Repr
  ( -- * Type-level descriptions of wire representations.
    ...
    -- * Mapping representations to value types from "Zap.Untyped"
    Untyped,
    UntypedData,
    UntypedPtr,
    UntypedSomePtr,
...
import Zap.TraversalLimit (evalLimitT)
import Zap.Untyped
  ( Allocate (..),
    DataSz (..),
    ElemRepr,
...
-- | @'ReprFor' a@ denotes the Zap wire represent of the type @a@.
...
-- | A @'Raw' mut a@ is an @a@ embedded in a zap message with mutability
-- @mut@.
...
-- | A phantom type denoting zap lists of type @a@.
data List a
